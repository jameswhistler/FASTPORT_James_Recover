if exists (select * from sysobjects where id = object_id(N'pFASTPORTFlowOverviewGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTFlowOverviewGetList 
GO
...
-- Returns a query resultset from table [dbo].[FlowOverview]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTFlowOverviewGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[FlowOverview] FlowOverview_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY FlowOverview_.[OverviewID] asc '
        ...
        SET @l_query_cols = 
            N'FlowOverview_.[OverviewID],
            FlowOverview_.[FlowID],
            FlowOverview_.[Overview],
            FlowOverview_.[OverviewRank],
            FlowOverview_.[OveriewTypeID],
            FlowOverview_.[CreatedByID],
            FlowOverview_.[CreatedAt],
            FlowOverview_.[UpdatedByID],
            FlowOverview_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(FlowOverview_.[OverviewID],FlowOverview_.[FlowID],FlowOverview_.[Overview],FlowOverview_.[OverviewRank],FlowOverview_.[OveriewTypeID],FlowOverview_.[CreatedByID],FlowOverview_.[CreatedAt],FlowOverview_.[UpdatedByID],FlowOverview_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [OverviewID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON FlowOverview_.[OverviewID] = IS_ALIAS.[OverviewID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(FlowOverview_.[OverviewID],FlowOverview_.[FlowID],FlowOverview_.[Overview],FlowOverview_.[OverviewRank],FlowOverview_.[OveriewTypeID],FlowOverview_.[CreatedByID],FlowOverview_.[CreatedAt],FlowOverview_.[UpdatedByID],FlowOverview_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[FlowOverview] FlowOverview_ ' + 
            'WHERE 1=2;'
