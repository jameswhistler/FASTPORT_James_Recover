if exists (select * from sysobjects where id = object_id(N'pFASTPORTPartyIncidentTicketedForGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTPartyIncidentTicketedForGetList 
GO
...
-- Returns a query resultset from table [dbo].[PartyIncidentTicketedFor]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTPartyIncidentTicketedForGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[PartyIncidentTicketedFor] PartyIncidentTicketedFor_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY PartyIncidentTicketedFor_.[TicketedID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'PartyIncidentTicketedFor_.[TicketedID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'PartyIncidentTicketedFor_.[TicketedID],
            PartyIncidentTicketedFor_.[IncidentID],
            PartyIncidentTicketedFor_.[TicketedForID],
            PartyIncidentTicketedFor_.[Speed],
            PartyIncidentTicketedFor_.[Limit],
            CAST(BINARY_CHECKSUM(PartyIncidentTicketedFor_.[TicketedID],PartyIncidentTicketedFor_.[IncidentID],PartyIncidentTicketedFor_.[TicketedForID],PartyIncidentTicketedFor_.[Speed],PartyIncidentTicketedFor_.[Limit]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [TicketedID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON PartyIncidentTicketedFor_.[TicketedID] = IS_ALIAS.[TicketedID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(PartyIncidentTicketedFor_.[TicketedID],PartyIncidentTicketedFor_.[IncidentID],PartyIncidentTicketedFor_.[TicketedForID],PartyIncidentTicketedFor_.[Speed],PartyIncidentTicketedFor_.[Limit]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[PartyIncidentTicketedFor] PartyIncidentTicketedFor_ ' + 
            'WHERE 1=2;'
