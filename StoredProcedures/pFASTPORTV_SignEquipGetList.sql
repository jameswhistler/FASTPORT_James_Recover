if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignEquipGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignEquipGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SignEquip]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SignEquipGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_SignEquip] V_SignEquip_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_SignEquip_.[EquipID] asc '
        ...
        SET @l_query_select = 'WITH V_SignEquip_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'V_SignEquip_.[EquipID],
            V_SignEquip_.[PartyID],
            V_SignEquip_.[OwnerID],
            V_SignEquip_.[O_Name],
            V_SignEquip_.[V_Info],
            V_SignEquip_.[V_Registration],
            V_SignEquip_.[V_Specs],
            V_SignEquip_.[V_PurchaseInfo],
            V_SignEquip_.[V_Year],
            V_SignEquip_.[V_Make],
            V_SignEquip_.[V_Model],
            V_SignEquip_.[V_Axels],
            V_SignEquip_.[V_UnladenWeight],
            V_SignEquip_.[V_Height],
            V_SignEquip_.[V_PurchasedDate],
            V_SignEquip_.[V_PurchasedValue] '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM V_SignEquip_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_SignEquip_.[V_PurchasedValue]'
        SET @l_query_from = 
            ' FROM [dbo].[v_SignEquip] V_SignEquip_ ' + 
            'WHERE 1=2;'
