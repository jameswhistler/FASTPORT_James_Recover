if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_GeoCityGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_GeoCityGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_GeoCity]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_GeoCityGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            N'V_GeoCity_.[CountryID],
            V_GeoCity_.[Country],
            V_GeoCity_.[CountryAbbr],
            V_GeoCity_.[StateID],
            V_GeoCity_.[CityST],
            V_GeoCity_.[CityID],
            V_GeoCity_.[City],
        ...
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CityID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_GeoCity_.[CityID] = IS_ALIAS.[CityID] ' 
