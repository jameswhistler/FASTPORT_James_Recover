if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_FellowUserDistinctRolesDrillDown') and sysstat & 0xf = 4) drop procedure pFASTPORTV_FellowUserDistinctRolesDrillDown 
GO
...
CREATE PROCEDURE pFASTPORTV_FellowUserDistinctRolesDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''v_FellowUserDistinctRoles'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
