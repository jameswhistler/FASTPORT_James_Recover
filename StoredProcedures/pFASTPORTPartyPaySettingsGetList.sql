if exists (select * from sysobjects where id = object_id(N'pFASTPORTPartyPaySettingsGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTPartyPaySettingsGetList 
GO
...
-- Returns a query resultset from table [dbo].[PartyPaySettings]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTPartyPaySettingsGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[PartyPaySettings] PartyPaySettings_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY PartyPaySettings_.[PaySettingsID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'PartyPaySettings_.[PaySettingsID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'PartyPaySettings_.[PaySettingsID],
            PartyPaySettings_.[PartyID],
            PartyPaySettings_.[PayPalToken],
            PartyPaySettings_.[FlowStepID],
            CAST(BINARY_CHECKSUM(PartyPaySettings_.[PaySettingsID],PartyPaySettings_.[PartyID],PartyPaySettings_.[PayPalToken],PartyPaySettings_.[FlowStepID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [PaySettingsID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON PartyPaySettings_.[PaySettingsID] = IS_ALIAS.[PaySettingsID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(PartyPaySettings_.[PaySettingsID],PartyPaySettings_.[PartyID],PartyPaySettings_.[PayPalToken],PartyPaySettings_.[FlowStepID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[PartyPaySettings] PartyPaySettings_ ' + 
            'WHERE 1=2;'
