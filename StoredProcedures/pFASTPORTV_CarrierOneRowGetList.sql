if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_CarrierOneRowGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_CarrierOneRowGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_CarrierOneRow]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_CarrierOneRowGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_CarrierOneRow_.[PartyID] asc '
        ...
        SET @l_query_select = 'WITH V_CarrierOneRow_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'V_CarrierOneRow_.[PK],
            V_CarrierOneRow_.[PartyID],
            V_CarrierOneRow_.[AddrID],
            V_CarrierOneRow_.[DOTNumber],
            V_CarrierOneRow_.[MCNumber],
            V_CarrierOneRow_.[CarrierName],
            V_CarrierOneRow_.[CarrierDBA],
            V_CarrierOneRow_.[Addr],
            V_CarrierOneRow_.[Addr2],
            V_CarrierOneRow_.[CitySTZip],
            V_CarrierOneRow_.[CountryID],
            V_CarrierOneRow_.[Country],
            V_CarrierOneRow_.[AddrZipID],
            V_CarrierOneRow_.[Email],
            V_CarrierOneRow_.[DirectDial],
            V_CarrierOneRow_.[Fax],
            V_CarrierOneRow_.[CarrierHTML],
            V_CarrierOneRow_.[StripSearch],
            V_CarrierOneRow_.[StripSearchDBA],
            V_CarrierOneRow_.[Lat],
            V_CarrierOneRow_.[Long],
            V_CarrierOneRow_.[CountryAbbr] '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM V_CarrierOneRow_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_CarrierOneRow_.[CountryAbbr]'
        SET @l_query_from = 
            ' FROM [dbo].[v_CarrierOneRow] V_CarrierOneRow_ ' + 
            'WHERE 1=2;'
