if exists (select * from sysobjects where id = object_id(N'pFASTPORTCalendarGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTCalendarGetList 
GO
...
-- Returns a query resultset from table [dbo].[Calendar]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTCalendarGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Calendar_.[CalID] asc '
        ...
        SET @l_query_cols = 
            ...
            Calendar_.[Holiday],
            Calendar_.[HolType],
            CAST(BINARY_CHECKSUM(Calendar_.[CalID],Calendar_.[CalType],Calendar_.[CalDate],Calendar_.[CountryID],Calendar_.[Holiday],Calendar_.[HolType]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CalID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Calendar_.[CalID] = IS_ALIAS.[CalID] ' 
