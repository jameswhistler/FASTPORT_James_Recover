if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_CarriersGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_CarriersGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_Carriers]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_CarriersGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_Carriers_.[PK] asc '
        ...
        SET @l_query_select = 'WITH V_Carriers_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            V_Carriers_.[Logo],
            V_Carriers_.[Carrier],
            V_Carriers_.[CarrierHTML],
            V_Carriers_.[CarrierNos],
            V_Carriers_.[Name],
            V_Carriers_.[CarrierName],
            V_Carriers_.[CarrierDBA],
            V_Carriers_.[AddrName],
            V_Carriers_.[Addr],
            V_Carriers_.[Addr2],
            V_Carriers_.[AddrZipID],
            V_Carriers_.[CitySTZip],
            V_Carriers_.[MCNumber],
            V_Carriers_.[DOTNumber],
            V_Carriers_.[StripSearch],
            V_Carriers_.[StripSearchDBA],
            V_Carriers_.[Lat],
            V_Carriers_.[Long] '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM V_Carriers_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_Carriers_.[Long]'
        SET @l_query_from = 
            ' FROM [dbo].[v_Carriers] V_Carriers_ ' + 
            'WHERE 1=2;'
