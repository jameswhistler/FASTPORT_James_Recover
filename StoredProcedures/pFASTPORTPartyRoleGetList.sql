if exists (select * from sysobjects where id = object_id(N'pFASTPORTPartyRoleGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTPartyRoleGetList 
GO
...
-- Returns a query resultset from table [dbo].[PartyRole]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTPartyRoleGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[PartyRole] PartyRole_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY PartyRole_.[PartyRoleID] asc '
        ...
        SET @l_query_select = 'WITH PartyRole_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'PartyRole_.[PartyRoleID],
            PartyRole_.[RollupID],
            PartyRole_.[PartyID],
            PartyRole_.[RoleID],
            PartyRole_.[CreatedByID],
            PartyRole_.[CreatedAt],
            PartyRole_.[UpdatedByID],
            PartyRole_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(PartyRole_.[PartyRoleID],PartyRole_.[RollupID],PartyRole_.[PartyID],PartyRole_.[RoleID],PartyRole_.[CreatedByID],PartyRole_.[CreatedAt],PartyRole_.[UpdatedByID],PartyRole_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM PartyRole_ '
