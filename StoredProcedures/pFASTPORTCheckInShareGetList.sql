if exists (select * from sysobjects where id = object_id(N'pFASTPORTCheckInShareGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTCheckInShareGetList 
GO
...
-- Returns a query resultset from table [dbo].[CheckInShare]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTCheckInShareGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[CheckInShare] CheckInShare_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY CheckInShare_.[CheckInShareID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'CheckInShare_.[CheckInShareID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'CheckInShare_.[CheckInShareID],
            CheckInShare_.[CheckInID],
            CheckInShare_.[PartyID],
            CheckInShare_.[FollowUntilComplete],
            CheckInShare_.[CreatedByID],
            CheckInShare_.[CreatedAt],
            CheckInShare_.[UpdatedByID],
            CheckInShare_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(CheckInShare_.[CheckInShareID],CheckInShare_.[CheckInID],CheckInShare_.[PartyID],CheckInShare_.[FollowUntilComplete],CheckInShare_.[CreatedByID],CheckInShare_.[CreatedAt],CheckInShare_.[UpdatedByID],CheckInShare_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CheckInShareID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON CheckInShare_.[CheckInShareID] = IS_ALIAS.[CheckInShareID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(CheckInShare_.[CheckInShareID],CheckInShare_.[CheckInID],CheckInShare_.[PartyID],CheckInShare_.[FollowUntilComplete],CheckInShare_.[CreatedByID],CheckInShare_.[CreatedAt],CheckInShare_.[UpdatedByID],CheckInShare_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[CheckInShare] CheckInShare_ ' + 
            'WHERE 1=2;'
