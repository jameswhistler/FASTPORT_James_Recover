if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PartyCarriersGetStats') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PartyCarriersGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pFASTPORTV_PartyCarriersGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''v_PartyCarriers'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[v_PartyCarriers] V_PartyCarriers_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
