if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_GeoCountryGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_GeoCountryGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_GeoCountry]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_GeoCountryGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_GeoCountry] V_GeoCountry_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_GeoCountry_.[CountryID] asc '
        ...
        SET @l_query_cols = 
            N'V_GeoCountry_.[CountryID],
            V_GeoCountry_.[Country],
            V_GeoCountry_.[CountryAbbr] '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CountryID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_GeoCountry_.[CountryID] = IS_ALIAS.[CountryID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_GeoCountry_.[CountryAbbr]'
        SET @l_query_from = 
            ' FROM [dbo].[v_GeoCountry] V_GeoCountry_ ' + 
            'WHERE 1=2;'
