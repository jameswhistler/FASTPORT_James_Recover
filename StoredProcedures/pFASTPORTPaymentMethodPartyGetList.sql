if exists (select * from sysobjects where id = object_id(N'pFASTPORTPaymentMethodPartyGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTPaymentMethodPartyGetList 
GO
...
-- Returns a query resultset from table [dbo].[PaymentMethodParty]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTPaymentMethodPartyGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[PaymentMethodParty] PaymentMethodParty_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY PaymentMethodParty_.[PaymentMethodPartyID] asc '
        ...
        SET @l_temp_cols = 
            N'[PaymentMethodPartyID]'
        SET @l_temp_select = 
            ') ' + 
            'SELECT ' + 
            'TOP ' + convert(varchar, @l_end_gen_row_num) + ' '
        SET @l_temp_colsWithAlias = 
            N'PaymentMethodParty_.[PaymentMethodPartyID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'PaymentMethodParty_.[PaymentMethodPartyID],
            PaymentMethodParty_.[PaymentMethodID],
            PaymentMethodParty_.[PartyID],
            PaymentMethodParty_.[CreatedByID],
            PaymentMethodParty_.[CreatedAt],
            PaymentMethodParty_.[UpdatedByID],
            PaymentMethodParty_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(PaymentMethodParty_.[PaymentMethodPartyID],PaymentMethodParty_.[PaymentMethodID],PaymentMethodParty_.[PartyID],PaymentMethodParty_.[CreatedByID],PaymentMethodParty_.[CreatedAt],PaymentMethodParty_.[UpdatedByID],PaymentMethodParty_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [PaymentMethodPartyID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON PaymentMethodParty_.[PaymentMethodPartyID] = IS_ALIAS.[PaymentMethodPartyID] ' 
