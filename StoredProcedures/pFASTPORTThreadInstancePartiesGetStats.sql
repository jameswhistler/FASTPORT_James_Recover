if exists (select * from sysobjects where id = object_id(N'pFASTPORTThreadInstancePartiesGetStats') and sysstat & 0xf = 4) drop procedure pFASTPORTThreadInstancePartiesGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pFASTPORTThreadInstancePartiesGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''ThreadInstanceParties'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[ThreadInstanceParties] ThreadInstanceParties_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
