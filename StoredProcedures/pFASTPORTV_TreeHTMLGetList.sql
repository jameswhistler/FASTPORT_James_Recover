if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_TreeHTMLGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_TreeHTMLGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_TreeHTML]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_TreeHTMLGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_TreeHTML_.[TreeID] asc '
        ...
        SET @l_query_cols = 
            N'V_TreeHTML_.[TreeID],
            V_TreeHTML_.[TreeParentID],
            V_TreeHTML_.[ItemName],
            V_TreeHTML_.[TreeHTML],
            V_TreeHTML_.[ItemRank],
            V_TreeHTML_.[FolderOnly] '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [TreeID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_TreeHTML_.[TreeID] = IS_ALIAS.[TreeID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_TreeHTML_.[FolderOnly]'
        SET @l_query_from = 
            ' FROM [dbo].[v_TreeHTML] V_TreeHTML_ ' + 
            'WHERE 1=2;'
