if exists (select * from sysobjects where id = object_id(N'pFASTPORTSprocLogGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTSprocLogGetList 
GO
...
-- Returns a query resultset from table [dbo].[SprocLog]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTSprocLogGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SprocLog_.[SprocLogID] asc '
        ...
        SET @l_query_cols = 
            ...
            SprocLog_.[SprocName],
            SprocLog_.[Params],
            SprocLog_.[JoinString],
            SprocLog_.[WhereString],
            SprocLog_.[SortString],
            CAST(BINARY_CHECKSUM(SprocLog_.[SprocLogID],SprocLog_.[LoggedAt],SprocLog_.[SprocName],SprocLog_.[Params],SprocLog_.[JoinString],SprocLog_.[WhereString],SprocLog_.[SortString]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [SprocLogID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON SprocLog_.[SprocLogID] = IS_ALIAS.[SprocLogID] ' 
