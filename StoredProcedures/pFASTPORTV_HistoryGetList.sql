if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_HistoryGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_HistoryGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_History]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_HistoryGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_History_.[HistoryID] asc '
        ...
        SET @l_query_cols = 
            N'V_History_.[HistoryID],
            V_History_.[PartyID],
            V_History_.[EmployerID],
            V_History_.[EmployerName],
            V_History_.[HistoryHTML],
            V_History_.[ExpHTML],
            V_History_.[ExpItemsHTML],
            V_History_.[StartMonthID],
            V_History_.[EndMonthID],
            V_History_.[Duration],
            V_History_.[EditButton],
            V_History_.[DeleteButton] '
        SET @l_query_from = 
            'FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON V_History_.[HistoryID] = IS_ALIAS.[HistoryID] ' 
