if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignHistoryGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignHistoryGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SignHistory]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SignHistoryGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_SignHistory] V_SignHistory_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_SignHistory_.[HistoryID] asc '
        ...
        SET @l_query_select = 'WITH V_SignHistory_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'V_SignHistory_.[HistoryID],
            V_SignHistory_.[PartyID],
            V_SignHistory_.[HistoryHTML],
            V_SignHistory_.[ExpGeneral],
            V_SignHistory_.[ExpEquipment],
            V_SignHistory_.[ExpCargo],
            V_SignHistory_.[ExpRegions],
            V_SignHistory_.[FirstSort],
            V_SignHistory_.[StartDate],
            V_SignHistory_.[EndDate] '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM V_SignHistory_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_SignHistory_.[EndDate]'
        SET @l_query_from = 
            ' FROM [dbo].[v_SignHistory] V_SignHistory_ ' + 
            'WHERE 1=2;'
