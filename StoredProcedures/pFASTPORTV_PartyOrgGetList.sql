if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PartyOrgGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PartyOrgGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_PartyOrg]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_PartyOrgGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_PartyOrg_.[AddrID] asc '
        ...
        SET @l_query_cols = 
            ...
            V_PartyOrg_.[Logo],
            V_PartyOrg_.[Company],
            V_PartyOrg_.[AddrName],
            V_PartyOrg_.[PointOfInterest],
            V_PartyOrg_.[ContactInfo],
            V_PartyOrg_.[Pins],
            V_PartyOrg_.[PinsSmall],
            V_PartyOrg_.[PointStats],
            V_PartyOrg_.[CityST] '
        SET @l_query_from = 
            'FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON V_PartyOrg_.[AddrID] = IS_ALIAS.[AddrID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_PartyOrg_.[CityST]'
        SET @l_query_from = 
            ' FROM [dbo].[v_PartyOrg] V_PartyOrg_ ' + 
            'WHERE 1=2;'
