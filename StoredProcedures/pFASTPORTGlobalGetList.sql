if exists (select * from sysobjects where id = object_id(N'pFASTPORTGlobalGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTGlobalGetList 
GO
...
-- Returns a query resultset from table [dbo].[Global]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTGlobalGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[Global] Global_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Global_.[GlobalID] asc '
        ...
        SET @l_query_cols = 
            N'Global_.[GlobalID],
            Global_.[GlobalName],
            Global_.[GlobalDescription],
            Global_.[GlobalFile],
            Global_.[GlobalValue],
            CAST(BINARY_CHECKSUM(Global_.[GlobalID],Global_.[GlobalName],Global_.[GlobalDescription],Global_.[GlobalFile],Global_.[GlobalValue]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [GlobalID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Global_.[GlobalID] = IS_ALIAS.[GlobalID] ' 
