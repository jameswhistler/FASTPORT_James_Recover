if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_HistoryLoadRWDrillDown') and sysstat & 0xf = 4) drop procedure pFASTPORTV_HistoryLoadRWDrillDown 
GO
...
CREATE PROCEDURE pFASTPORTV_HistoryLoadRWDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''v_HistoryLoadRW'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
