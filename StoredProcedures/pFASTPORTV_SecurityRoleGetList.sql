if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SecurityRoleGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SecurityRoleGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SecurityRole]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SecurityRoleGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_SecurityRole_.[PartyRoleID] asc '
        ...
        SET @l_query_cols = 
            N'V_SecurityRole_.[PartyRoleID],
            V_SecurityRole_.[RollupID],
            V_SecurityRole_.[PartyID],
            V_SecurityRole_.[RoleID],
            V_SecurityRole_.[CreatedByID],
            V_SecurityRole_.[CreatedAt],
            V_SecurityRole_.[UpdatedByID],
            V_SecurityRole_.[UpdatedAt] '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [PartyRoleID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_SecurityRole_.[PartyRoleID] = IS_ALIAS.[PartyRoleID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_SecurityRole_.[UpdatedAt]'
        SET @l_query_from = 
            ' FROM [dbo].[v_SecurityRole] V_SecurityRole_ ' + 
            'WHERE 1=2;'
