if exists (select * from sysobjects where id = object_id(N'pFASTPORTThreadInstanceMessageAttachmentsDrillDown') and sysstat & 0xf = 4) drop procedure pFASTPORTThreadInstanceMessageAttachmentsDrillDown 
GO
...
CREATE PROCEDURE pFASTPORTThreadInstanceMessageAttachmentsDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''ThreadInstanceMessageAttachments'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ...
            'AS [ThreadInstanceMessageAttachments_]' +
