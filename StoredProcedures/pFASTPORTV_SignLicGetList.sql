if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignLicGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignLicGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SignLic]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SignLicGetList
...
BEGIN
    ...
    [DocNotes] varchar(2000) COLLATE SQL_Latin1_General_CP1_CI_AS
        ...
        SET @l_temp_colsWithAlias = 
            N'V_SignLic_.[DocID],
            V_SignLic_.[PartyID],
            V_SignLic_.[FiledAsID],
            V_SignLic_.[DocInfo],
            V_SignLic_.[DocShort],
            V_SignLic_.[DocNumber],
            V_SignLic_.[DocIssued],
            V_SignLic_.[DocIssuingState],
            V_SignLic_.[DocExpiration],
            V_SignLic_.[Reissued],
            V_SignLic_.[DocEndorsements],
            V_SignLic_.[DocNotes]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
