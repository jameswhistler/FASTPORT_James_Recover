if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignAddrGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignAddrGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SignAddr]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SignAddrGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH V_SignAddr_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'V_SignAddr_.[AddrID],
            V_SignAddr_.[PartyID],
            V_SignAddr_.[AddrHTML],
            V_SignAddr_.[Addr],
            V_SignAddr_.[City],
            V_SignAddr_.[ST],
            V_SignAddr_.[Zip],
        ...
        SET @l_query_select2 = 'SELECT * FROM V_SignAddr_ '
