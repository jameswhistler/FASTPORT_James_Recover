if exists (select * from sysobjects where id = object_id(N'pFASTPORTSystemJobLogGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTSystemJobLogGetList 
GO
...
-- Returns a query resultset from table [dbo].[SystemJobLog]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTSystemJobLogGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SystemJobLog] SystemJobLog_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SystemJobLog_.[JobLogID] asc '
        ...
        SET @l_query_cols = 
            N'SystemJobLog_.[JobLogID],
            SystemJobLog_.[JobID],
            SystemJobLog_.[JobStarted],
            SystemJobLog_.[JobCompleted],
            SystemJobLog_.[LogMessage],
            CAST(BINARY_CHECKSUM(SystemJobLog_.[JobLogID],SystemJobLog_.[JobID],SystemJobLog_.[JobStarted],SystemJobLog_.[JobCompleted],SystemJobLog_.[LogMessage]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [JobLogID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON SystemJobLog_.[JobLogID] = IS_ALIAS.[JobLogID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(SystemJobLog_.[JobLogID],SystemJobLog_.[JobID],SystemJobLog_.[JobStarted],SystemJobLog_.[JobCompleted],SystemJobLog_.[LogMessage]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[SystemJobLog] SystemJobLog_ ' + 
            'WHERE 1=2;'
