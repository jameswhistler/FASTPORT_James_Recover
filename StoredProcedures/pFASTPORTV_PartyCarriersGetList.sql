if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PartyCarriersGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PartyCarriersGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_PartyCarriers]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_PartyCarriersGetList
...
BEGIN
    ...
    BEGIN
        ...
        IS_ROWNUM_COL int identity(1,1),
                [PartyID] int,
    [CarrierID] int,
    [CarrierFullName] varchar(150) COLLATE SQL_Latin1_General_CP1_CI_AS
        ); '
        ...
        SET @l_temp_colsWithAlias = 
            N'V_PartyCarriers_.[PartyID],
            V_PartyCarriers_.[CarrierID],
            V_PartyCarriers_.[CarrierFullName]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
