if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignDocsGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignDocsGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_SignDocs]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_SignDocsGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_SignDocs] V_SignDocs_'
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH V_SignDocs_ AS ( SELECT  '
        ...
        SET @l_query_select2 = 'SELECT * FROM V_SignDocs_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            N'V_SignDocs_.[DocID],
            V_SignDocs_.[PartyID],
            V_SignDocs_.[FiledAsID],
            V_SignDocs_.[DocInfo],
            V_SignDocs_.[DocShort],
            V_SignDocs_.[DocNumber],
            V_SignDocs_.[DocIssued],
            V_SignDocs_.[DocIssuingState],
            V_SignDocs_.[DocExpiration],
            V_SignDocs_.[Reissued],
            V_SignDocs_.[DocEndorsements],
            V_SignDocs_.[DocNotes]'
        SET @l_query_from = 
            ' FROM [dbo].[v_SignDocs] V_SignDocs_ ' + 
            'WHERE 1=2;'
