if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_CarrierSimpleGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_CarrierSimpleGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_CarrierSimple]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_CarrierSimpleGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_CarrierSimple] V_CarrierSimple_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_CarrierSimple_.[CarrierID] asc '
        ...
        SET @l_query_cols = 
            N'V_CarrierSimple_.[CarrierID],
            V_CarrierSimple_.[PartyID],
            V_CarrierSimple_.[Logo],
            V_CarrierSimple_.[MajorStickers],
            V_CarrierSimple_.[MinorStickers],
            V_CarrierSimple_.[Carrier],
            V_CarrierSimple_.[CarrierHTML],
            V_CarrierSimple_.[CarrierProfile],
            V_CarrierSimple_.[ShortStats],
            V_CarrierSimple_.[GeneralStats],
            V_CarrierSimple_.[Name],
            V_CarrierSimple_.[MCNumber],
            V_CarrierSimple_.[DOTNumber],
            V_CarrierSimple_.[StripSearch] '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CarrierID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_CarrierSimple_.[CarrierID] = IS_ALIAS.[CarrierID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_CarrierSimple_.[StripSearch]'
        SET @l_query_from = 
            ' FROM [dbo].[v_CarrierSimple] V_CarrierSimple_ ' + 
            'WHERE 1=2;'
