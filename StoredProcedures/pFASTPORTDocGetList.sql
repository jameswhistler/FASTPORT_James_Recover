if exists (select * from sysobjects where id = object_id(N'pFASTPORTDocGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTDocGetList 
GO
...
-- Returns a query resultset from table [dbo].[Doc]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTDocGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Doc_.[DocID] asc '
        ...
        SET @l_query_cols = 
            ...
            Doc_.[DocNumber],
            Doc_.[DocIssued],
            Doc_.[DocIssuingStateID],
            Doc_.[DocExpiration],
            Doc_.[Reissued],
            Doc_.[DocEndorsements],
            Doc_.[DocNotes],
            Doc_.[FinishedRecordingDetails],
            Doc_.[PrivateFile],
            Doc_.[FlowStepID],
            Doc_.[InstanceID],
            Doc_.[ExecutionID],
            ...
            Doc_.[UpdatedByID],
            Doc_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(Doc_.[DocID],Doc_.[CIX],Doc_.[PartyID],Doc_.[FiledAsID],Doc_.[AgreementID],Doc_.[DocName],Doc_.[DocPdf],Doc_.[DocNumber],Doc_.[DocIssued],Doc_.[DocIssuingStateID],Doc_.[DocExpiration],Doc_.[Reissued],Doc_.[DocEndorsements],Doc_.[DocNotes],Doc_.[FinishedRecordingDetails],Doc_.[PrivateFile],Doc_.[FlowStepID],Doc_.[InstanceID],Doc_.[ExecutionID],Doc_.[AttachmentID],Doc_.[EquipID],Doc_.[ProcessedPages],Doc_.[CreatedByID],Doc_.[CreatedAt],Doc_.[UpdatedByID],Doc_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [DocID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Doc_.[DocID] = IS_ALIAS.[DocID] ' 
