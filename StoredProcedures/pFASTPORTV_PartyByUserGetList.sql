if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PartyByUserGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PartyByUserGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_PartyByUser]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_PartyByUserGetList
...
BEGIN
    ...
    [Handle] varchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS,
    [UserID] int
        ); '
        ...
        SET @l_temp_colsWithAlias = 
            N'V_PartyByUser_.[PartyID],
            V_PartyByUser_.[Name],
            V_PartyByUser_.[Logo],
            V_PartyByUser_.[ProfilePicture],
            V_PartyByUser_.[Handle],
            V_PartyByUser_.[UserID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
