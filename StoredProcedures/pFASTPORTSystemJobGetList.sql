if exists (select * from sysobjects where id = object_id(N'pFASTPORTSystemJobGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTSystemJobGetList 
GO
...
-- Returns a query resultset from table [dbo].[SystemJob]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTSystemJobGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SystemJob_.[JobID] asc '
        ...
        SET @l_query_cols = 
            N'SystemJob_.[JobID],
            SystemJob_.[SprocName],
            SystemJob_.[HourToRun],
            SystemJob_.[Param1],
            SystemJob_.[Param2],
            SystemJob_.[Param3],
            SystemJob_.[Param4],
            SystemJob_.[Param5],
            SystemJob_.[Param6],
            CAST(BINARY_CHECKSUM(SystemJob_.[JobID],SystemJob_.[SprocName],SystemJob_.[HourToRun],SystemJob_.[Param1],SystemJob_.[Param2],SystemJob_.[Param3],SystemJob_.[Param4],SystemJob_.[Param5],SystemJob_.[Param6]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [JobID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON SystemJob_.[JobID] = IS_ALIAS.[JobID] ' 
