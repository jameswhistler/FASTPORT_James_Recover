if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_AdDetailsGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_AdDetailsGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_AdDetails]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_AdDetailsGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_AdDetails] V_AdDetails_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_AdDetails_.[AdID] asc '
        ...
        SET @l_query_select = 'WITH V_AdDetails_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'V_AdDetails_.[AdID],
            V_AdDetails_.[PartyID],
            V_AdDetails_.[CarrierID],
            V_AdDetails_.[AdHTML],
            V_AdDetails_.[LongDescription],
            V_AdDetails_.[GeneralExperience],
            V_AdDetails_.[EquipExperience],
            V_AdDetails_.[CargoExperience],
            V_AdDetails_.[RegionalExperience],
            V_AdDetails_.[Requirements],
            V_AdDetails_.[PayHTML] '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM V_AdDetails_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_AdDetails_.[PayHTML]'
        SET @l_query_from = 
            ' FROM [dbo].[v_AdDetails] V_AdDetails_ ' + 
            'WHERE 1=2;'
