if exists (select * from sysobjects where id = object_id(N'pFASTPORTCarrierAdGeoGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTCarrierAdGeoGetList 
GO
...
-- Returns a query resultset from table [dbo].[CarrierAdGeo]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTCarrierAdGeoGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[CarrierAdGeo] CarrierAdGeo_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY CarrierAdGeo_.[AdGeoID] asc '
        ...
        SET @l_query_select = 'WITH CarrierAdGeo_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'CarrierAdGeo_.[AdGeoID],
            CarrierAdGeo_.[AdID],
            CarrierAdGeo_.[AdGeoTypeID],
            CarrierAdGeo_.[OriginCityID],
            CarrierAdGeo_.[OriginRadius],
            CarrierAdGeo_.[DestRadius],
            CarrierAdGeo_.[AdGeoNotes],
            CAST(BINARY_CHECKSUM(CarrierAdGeo_.[AdGeoID],CarrierAdGeo_.[AdID],CarrierAdGeo_.[AdGeoTypeID],CarrierAdGeo_.[OriginCityID],CarrierAdGeo_.[OriginRadius],CarrierAdGeo_.[DestRadius],CarrierAdGeo_.[AdGeoNotes]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM CarrierAdGeo_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(CarrierAdGeo_.[AdGeoID],CarrierAdGeo_.[AdID],CarrierAdGeo_.[AdGeoTypeID],CarrierAdGeo_.[OriginCityID],CarrierAdGeo_.[OriginRadius],CarrierAdGeo_.[DestRadius],CarrierAdGeo_.[AdGeoNotes]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[CarrierAdGeo] CarrierAdGeo_ ' + 
            'WHERE 1=2;'
