if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_AsposeTestGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_AsposeTestGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_AsposeTest]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_AsposeTestGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_AsposeTest] V_AsposeTest_'
    ...
    [SenderID] int,
    [SenderPic] image,
    [SenderName] varchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS,
    [RecipientName] varchar(50) COLLATE SQL_Latin1_General_CP1_CI_AS,
    [RecipientSignature] image,
    [TestHTML] nvarchar(4000) COLLATE SQL_Latin1_General_CP1_CI_AS
        ); '
        ...
        SET @l_temp_colsWithAlias = 
            N'V_AsposeTest_.[ExecutionID],
            V_AsposeTest_.[SenderID],
            V_AsposeTest_.[SenderPic],
            V_AsposeTest_.[SenderName],
            V_AsposeTest_.[RecipientName],
            V_AsposeTest_.[RecipientSignature],
            V_AsposeTest_.[TestHTML]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_AsposeTest_.[TestHTML]'
        SET @l_query_from = 
            ' FROM [dbo].[v_AsposeTest] V_AsposeTest_ ' + 
            'WHERE 1=2;'
