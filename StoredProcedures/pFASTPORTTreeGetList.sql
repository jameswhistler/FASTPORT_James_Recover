if exists (select * from sysobjects where id = object_id(N'pFASTPORTTreeGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTTreeGetList 
GO
...
-- Returns a query resultset from table [dbo].[Tree]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTTreeGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Tree_.[TreeID] asc '
        ...
        SET @l_query_select = 'WITH Tree_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            Tree_.[ItemImageGray],
            Tree_.[ImageGrayPath],
            Tree_.[ItemImageLowRes],
            Tree_.[ImageLowResPath],
            Tree_.[FolderOnly],
            Tree_.[ItemRank],
            Tree_.[Hide],
            Tree_.[AdminOnly],
            ...
            Tree_.[UpdatedAt],
            Tree_.[ExternalID],
            CAST(BINARY_CHECKSUM(Tree_.[TreeID],Tree_.[CIX],Tree_.[TreeParentID],Tree_.[ItemName],Tree_.[Description],Tree_.[Abbreviation],Tree_.[ItemImage],Tree_.[ImagePath],Tree_.[ItemImageGlow],Tree_.[ImageGlowPath],Tree_.[ItemImageGray],Tree_.[ImageGrayPath],Tree_.[ItemImageLowRes],Tree_.[ImageLowResPath],Tree_.[FolderOnly],Tree_.[ItemRank],Tree_.[Hide],Tree_.[AdminOnly],Tree_.[GeoStateID],Tree_.[GeoMarketID],Tree_.[NoSlide],Tree_.[YearSlider],Tree_.[CreatedByID],Tree_.[CreatedAt],Tree_.[UpdatedByID],Tree_.[UpdatedAt],Tree_.[ExternalID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM Tree_ '
