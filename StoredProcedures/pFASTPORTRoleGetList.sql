if exists (select * from sysobjects where id = object_id(N'pFASTPORTRoleGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTRoleGetList 
GO
...
-- Returns a query resultset from table [dbo].[Role]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTRoleGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Role_.[RoleID] asc '
        ...
        SET @l_query_cols = 
            ...
            Role_.[RoleTypeID],
            Role_.[Role],
            Role_.[RoleDescription],
            Role_.[RoleRank],
            CAST(BINARY_CHECKSUM(Role_.[RoleID],Role_.[GeneralRoleID],Role_.[RoleTypeID],Role_.[Role],Role_.[RoleDescription],Role_.[RoleRank]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON Role_.[RoleID] = IS_ALIAS.[RoleID] ' 
