if exists (select * from sysobjects where id = object_id(N'pFASTPORTPartyCarrierGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTPartyCarrierGetList 
GO
...
-- Returns a query resultset from table [dbo].[PartyCarrier]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTPartyCarrierGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[PartyCarrier] PartyCarrier_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY PartyCarrier_.[CarrierID] asc '
        ...
        SET @l_query_select = 'WITH PartyCarrier_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'PartyCarrier_.[CarrierID],
            PartyCarrier_.[PartyID],
            PartyCarrier_.[CarrierFullName],
            PartyCarrier_.[DOTNumber],
            PartyCarrier_.[MCNumber],
            PartyCarrier_.[Drivers],
            PartyCarrier_.[Certified],
            PartyCarrier_.[PayCertified],
            CAST(BINARY_CHECKSUM(PartyCarrier_.[CarrierID],PartyCarrier_.[PartyID],PartyCarrier_.[CarrierFullName],PartyCarrier_.[DOTNumber],PartyCarrier_.[MCNumber],PartyCarrier_.[Drivers],PartyCarrier_.[Certified],PartyCarrier_.[PayCertified]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM PartyCarrier_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(PartyCarrier_.[CarrierID],PartyCarrier_.[PartyID],PartyCarrier_.[CarrierFullName],PartyCarrier_.[DOTNumber],PartyCarrier_.[MCNumber],PartyCarrier_.[Drivers],PartyCarrier_.[Certified],PartyCarrier_.[PayCertified]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[PartyCarrier] PartyCarrier_ ' + 
            'WHERE 1=2;'
