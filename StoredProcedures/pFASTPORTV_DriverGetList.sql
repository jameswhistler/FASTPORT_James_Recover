if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_DriverGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_DriverGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_Driver]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_DriverGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[v_Driver] V_Driver_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_Driver_.[PartyID] asc '
        ...
        SET @l_query_cols = 
            ...
            V_Driver_.[AdminHTML],
            V_Driver_.[Details],
            V_Driver_.[ProfileSnippet],
            V_Driver_.[FullProfile],
            V_Driver_.[MajorStickers],
            V_Driver_.[MinorStickers],
            V_Driver_.[GeneralStats],
            V_Driver_.[YearsTrucking],
            V_Driver_.[SocialStatus],
            V_Driver_.[GeneralExperience],
            V_Driver_.[EquipExperience],
            V_Driver_.[CargoExperience],
            V_Driver_.[RegionalExperience],
            V_Driver_.[Accidents],
            V_Driver_.[OtherRecords],
            V_Driver_.[Warnings],
            V_Driver_.[AllRecords],
            V_Driver_.[GaugeID] '
        SET @l_query_from = 
            'FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON V_Driver_.[PartyID] = IS_ALIAS.[PartyID] ' 
