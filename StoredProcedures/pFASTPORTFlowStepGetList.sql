if exists (select * from sysobjects where id = object_id(N'pFASTPORTFlowStepGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTFlowStepGetList 
GO
...
-- Returns a query resultset from table [dbo].[FlowStep]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTFlowStepGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[FlowStep] FlowStep_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY FlowStep_.[FlowStepID] asc '
        ...
        SET @l_query_cols = 
            N'FlowStep_.[FlowStepID],
            FlowStep_.[FlowCollectionID],
            FlowStep_.[OverviewID],
            FlowStep_.[StepRank],
            FlowStep_.[RefName],
            FlowStep_.[FlowStatus],
            FlowStep_.[ReturnedDocStep],
            FlowStep_.[DisableStep],
            FlowStep_.[CreatedByID],
            FlowStep_.[CreatedAt],
            FlowStep_.[UpdatedByID],
            FlowStep_.[UpdatedAt],
            FlowStep_.[CopyFlowStepID],
            CAST(BINARY_CHECKSUM(FlowStep_.[FlowStepID],FlowStep_.[FlowCollectionID],FlowStep_.[OverviewID],FlowStep_.[StepRank],FlowStep_.[RefName],FlowStep_.[FlowStatus],FlowStep_.[ReturnedDocStep],FlowStep_.[DisableStep],FlowStep_.[CreatedByID],FlowStep_.[CreatedAt],FlowStep_.[UpdatedByID],FlowStep_.[UpdatedAt],FlowStep_.[CopyFlowStepID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [FlowStepID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON FlowStep_.[FlowStepID] = IS_ALIAS.[FlowStepID] ' 
