if exists (select * from sysobjects where id = object_id(N'pFASTPORTEquipGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTEquipGetList 
GO
...
-- Returns a query resultset from table [dbo].[Equip]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTEquipGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[Equip] Equip_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Equip_.[EquipID] asc '
        ...
        SET @l_query_cols = 
            ...
            Equip_.[Model],
            Equip_.[Axels],
            Equip_.[UnladenWeight],
            Equip_.[HeightFeet],
            Equip_.[HeightInches],
            Equip_.[PurchasedDate],
            Equip_.[PurchasedValue],
            CAST(BINARY_CHECKSUM(Equip_.[EquipID],Equip_.[PartyID],Equip_.[OwnerID],Equip_.[YearID],Equip_.[Make],Equip_.[Model],Equip_.[Axels],Equip_.[UnladenWeight],Equip_.[HeightFeet],Equip_.[HeightInches],Equip_.[PurchasedDate],Equip_.[PurchasedValue]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [EquipID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Equip_.[EquipID] = IS_ALIAS.[EquipID] ' 
