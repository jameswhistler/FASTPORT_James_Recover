if exists (select * from sysobjects where id = object_id(N'pFASTPORTCarrierAdContactsGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTCarrierAdContactsGetList 
GO
...
-- Returns a query resultset from table [dbo].[CarrierAdContacts]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTCarrierAdContactsGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[CarrierAdContacts] CarrierAdContacts_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY CarrierAdContacts_.[AdContactsID] asc '
        ...
        SET @l_query_select = 'WITH CarrierAdContacts_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'CarrierAdContacts_.[AdContactsID],
            CarrierAdContacts_.[AdID],
            CarrierAdContacts_.[ContactTypeID],
            CarrierAdContacts_.[RoleID],
            CarrierAdContacts_.[PartyID],
            CAST(BINARY_CHECKSUM(CarrierAdContacts_.[AdContactsID],CarrierAdContacts_.[AdID],CarrierAdContacts_.[ContactTypeID],CarrierAdContacts_.[RoleID],CarrierAdContacts_.[PartyID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM CarrierAdContacts_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(CarrierAdContacts_.[AdContactsID],CarrierAdContacts_.[AdID],CarrierAdContacts_.[ContactTypeID],CarrierAdContacts_.[RoleID],CarrierAdContacts_.[PartyID]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[CarrierAdContacts] CarrierAdContacts_ ' + 
            'WHERE 1=2;'
