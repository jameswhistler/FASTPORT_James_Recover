if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_CaseTreeGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_CaseTreeGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_CaseTree]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_CaseTreeGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_CaseTree_.[UseCaseID] asc '
        ...
        SET @l_query_cols = 
            N'V_CaseTree_.[UseCaseID],
            V_CaseTree_.[CaseParentID],
            V_CaseTree_.[CaseSort],
            V_CaseTree_.[CaseName],
            V_CaseTree_.[CaseDescription],
            V_CaseTree_.[VideoURL],
            V_CaseTree_.[AppPage],
            V_CaseTree_.[AppPageTitle],
            V_CaseTree_.[Button1],
            V_CaseTree_.[Button1Tip],
            V_CaseTree_.[RedirectURL],
            V_CaseTree_.[Button2],
            V_CaseTree_.[Button2Tip],
            V_CaseTree_.[RedirectURL2],
            V_CaseTree_.[Button3],
            V_CaseTree_.[Button3Tip],
            V_CaseTree_.[RedirectURL3],
            V_CaseTree_.[CrossCaseID],
            V_CaseTree_.[CrossCase],
            V_CaseTree_.[Tallys],
            V_CaseTree_.[OverviewImage],
            V_CaseTree_.[AlternateFlowYN],
            V_CaseTree_.[FlowCount],
            V_CaseTree_.[AltCount],
            V_CaseTree_.[RuleCount],
            V_CaseTree_.[FieldsCount],
            V_CaseTree_.[QuestionsCount],
            V_CaseTree_.[SupportCount],
            V_CaseTree_.[HistoryCount] '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [UseCaseID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON V_CaseTree_.[UseCaseID] = IS_ALIAS.[UseCaseID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_CaseTree_.[HistoryCount]'
        SET @l_query_from = 
            ' FROM [dbo].[v_CaseTree] V_CaseTree_ ' + 
            'WHERE 1=2;'
