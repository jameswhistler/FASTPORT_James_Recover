if exists (select * from sysobjects where id = object_id(N'pFASTPORTRefGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTRefGetList 
GO
...
-- Returns a query resultset from table [dbo].[Ref]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTRefGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Ref_.[RefID] asc '
        ...
        SET @l_query_cols = 
            ...
            Ref_.[UpdatedByID],
            Ref_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(Ref_.[RefID],Ref_.[RefTypeID],Ref_.[Ref],Ref_.[OrdID],Ref_.[PartyID],Ref_.[PartyUserID],Ref_.[DocID],Ref_.[IssuedAt],Ref_.[ExpiresOn],Ref_.[CreatedByID],Ref_.[CreatedAt],Ref_.[UpdatedByID],Ref_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [RefID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Ref_.[RefID] = IS_ALIAS.[RefID] ' 
