if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SecurityUserGetStats') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SecurityUserGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pFASTPORTV_SecurityUserGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''v_SecurityUser'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        SET @l_from_str = '[dbo].[v_SecurityUser] V_SecurityUser_'
        ...
        BEGIN
            ...
            ELSE
                SET @l_sort_str = N'ORDER BY V_SecurityUser_.[PartyID] asc '
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[v_SecurityUser] V_SecurityUser_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
