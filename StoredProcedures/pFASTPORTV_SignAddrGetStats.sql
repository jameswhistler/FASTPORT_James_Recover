if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_SignAddrGetStats') and sysstat & 0xf = 4) drop procedure pFASTPORTV_SignAddrGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pFASTPORTV_SignAddrGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''v_SignAddr'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        SET @l_from_str = '[dbo].[v_SignAddr] V_SignAddr_'
        ...
        BEGIN
            ...
            ELSE
                SET @l_sort_str = N'ORDER BY V_SignAddr_.[AddrID] asc '
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[v_SignAddr] V_SignAddr_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
