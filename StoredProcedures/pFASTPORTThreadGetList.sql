if exists (select * from sysobjects where id = object_id(N'pFASTPORTThreadGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTThreadGetList 
GO
...
-- Returns a query resultset from table [dbo].[Thread]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTThreadGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[Thread] Thread_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Thread_.[ThreadID] asc '
        ...
        SET @l_query_cols = 
            N'Thread_.[ThreadID],
            Thread_.[ThreadTreeID],
            Thread_.[ThreadTypeID],
            Thread_.[ThreadName],
            Thread_.[FlowCollectionID],
            Thread_.[RelationTypeID],
            Thread_.[FirstSubject],
            Thread_.[FirstAction],
            Thread_.[FirstButtonText],
            Thread_.[NoRadWindow],
            Thread_.[GoAction],
            Thread_.[FirstButtonPage],
            Thread_.[FirstBody],
            Thread_.[CreatedByID],
            Thread_.[CreatedAt],
            Thread_.[UpdatedByID],
            Thread_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(Thread_.[ThreadID],Thread_.[ThreadTreeID],Thread_.[ThreadTypeID],Thread_.[ThreadName],Thread_.[FlowCollectionID],Thread_.[RelationTypeID],Thread_.[FirstSubject],Thread_.[FirstAction],Thread_.[FirstButtonText],Thread_.[NoRadWindow],Thread_.[GoAction],Thread_.[FirstButtonPage],Thread_.[FirstBody],Thread_.[CreatedByID],Thread_.[CreatedAt],Thread_.[UpdatedByID],Thread_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [ThreadID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON Thread_.[ThreadID] = IS_ALIAS.[ThreadID] ' 
