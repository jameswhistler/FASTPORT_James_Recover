if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PartyUserRolesGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PartyUserRolesGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_PartyUserRoles]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_PartyUserRolesGetList
...
BEGIN
    ...
    BEGIN
        ...
        IS_ROWNUM_COL int identity(1,1),
                [PartyID] int,
    [Name] varchar(1000) COLLATE SQL_Latin1_General_CP1_CI_AS,
    [UserID] int,
    [RoleID] int
        ); '
        ...
        SET @l_temp_colsWithAlias = 
            N'V_PartyUserRoles_.[PartyID],
            V_PartyUserRoles_.[Name],
            V_PartyUserRoles_.[UserID],
            V_PartyUserRoles_.[RoleID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
