if exists (select * from sysobjects where id = object_id(N'pFASTPORTCheckInActivityGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTCheckInActivityGetList 
GO
...
-- Returns a query resultset from table [dbo].[CheckInActivity]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTCheckInActivityGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[CheckInActivity] CheckInActivity_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY CheckInActivity_.[CheckInActivityID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'CheckInActivity_.[CheckInActivityID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'CheckInActivity_.[CheckInActivityID],
            CheckInActivity_.[CheckInID],
            CheckInActivity_.[ActivityID],
            CheckInActivity_.[CreatedByID],
            CheckInActivity_.[CreatedAt],
            CheckInActivity_.[UpdatedByID],
            CheckInActivity_.[UpdatedAt],
            CAST(BINARY_CHECKSUM(CheckInActivity_.[CheckInActivityID],CheckInActivity_.[CheckInID],CheckInActivity_.[ActivityID],CheckInActivity_.[CreatedByID],CheckInActivity_.[CreatedAt],CheckInActivity_.[UpdatedByID],CheckInActivity_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [CheckInActivityID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON CheckInActivity_.[CheckInActivityID] = IS_ALIAS.[CheckInActivityID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(CheckInActivity_.[CheckInActivityID],CheckInActivity_.[CheckInID],CheckInActivity_.[ActivityID],CheckInActivity_.[CreatedByID],CheckInActivity_.[CreatedAt],CheckInActivity_.[UpdatedByID],CheckInActivity_.[UpdatedAt]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[CheckInActivity] CheckInActivity_ ' + 
            'WHERE 1=2;'
