if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_PersonSnapshotGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_PersonSnapshotGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_PersonSnapshot]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_PersonSnapshotGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY V_PersonSnapshot_.[PartyID] asc '
        ...
        SET @l_query_cols = 
            N'V_PersonSnapshot_.[PartyID],
            V_PersonSnapshot_.[FromPic],
            V_PersonSnapshot_.[NameHTML],
            V_PersonSnapshot_.[MoreHTML],
            V_PersonSnapshot_.[Details],
            V_PersonSnapshot_.[ProfileSnippet],
            V_PersonSnapshot_.[FullProfile],
            V_PersonSnapshot_.[MajorStickers],
            V_PersonSnapshot_.[MinorStickers],
            V_PersonSnapshot_.[GeneralStats],
            V_PersonSnapshot_.[SocialStatus] '
        SET @l_query_from = 
            'FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON V_PersonSnapshot_.[PartyID] = IS_ALIAS.[PartyID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            V_PersonSnapshot_.[SocialStatus]'
        SET @l_query_from = 
            ' FROM [dbo].[v_PersonSnapshot] V_PersonSnapshot_ ' + 
            'WHERE 1=2;'
