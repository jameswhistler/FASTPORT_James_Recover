if exists (select * from sysobjects where id = object_id(N'pFASTPORTAudit_CarrierAdActivityGetStats') and sysstat & 0xf = 4) drop procedure pFASTPORTAudit_CarrierAdActivityGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pFASTPORTAudit_CarrierAdActivityGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''Audit_CarrierAdActivity'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[Audit_CarrierAdActivity] Audit_CarrierAdActivity_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
