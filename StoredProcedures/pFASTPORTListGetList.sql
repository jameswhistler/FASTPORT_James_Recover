if exists (select * from sysobjects where id = object_id(N'pFASTPORTListGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTListGetList 
GO
...
-- Returns a query resultset from table [dbo].[List]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTListGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY List_.[ListID] asc '
        ...
        SET @l_query_cols = 
            ...
            List_.[ListRank],
            List_.[ListValue],
            CAST(BINARY_CHECKSUM(List_.[ListID],List_.[ListType],List_.[ListName],List_.[ListRank],List_.[ListValue]) AS nvarchar(4000)) AS IS_CHECKSUM_COLUMN_12345 '
        SET @l_query_from = 
            'FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [ListID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON List_.[ListID] = IS_ALIAS.[ListID] ' 
