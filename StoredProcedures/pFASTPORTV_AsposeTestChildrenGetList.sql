if exists (select * from sysobjects where id = object_id(N'pFASTPORTV_AsposeTestChildrenGetList') and sysstat & 0xf = 4) drop procedure pFASTPORTV_AsposeTestChildrenGetList 
GO
...
-- Returns a query resultset from table [dbo].[v_AsposeTestChildren]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pFASTPORTV_AsposeTestChildrenGetList
...
BEGIN
    ...
    BEGIN
        ...
        IS_ROWNUM_COL int identity(1,1),
                [SenderID] int,
    [ItemImage] image,
    [ItemName] varchar(100) COLLATE SQL_Latin1_General_CP1_CI_AS
        ); '
        ...
        SET @l_temp_colsWithAlias = 
            N'V_AsposeTestChildren_.[SenderID],
            V_AsposeTestChildren_.[ItemImage],
            V_AsposeTestChildren_.[ItemName]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
