...
Partial Class Home
    ...
    Public Function f_ProcessURL(ByVal myArgs As String) As String
        ...
        Dim my6th As String = CustGenClass.f_Split_ByComma(myArgs, 7)

        Dim myReturn As String = "Nothing"

        If myAction = "LoadGeoJson" Or myAction = "LoadSelection" Then

            myReturn = CustGenClass.f_Sproc("usp_GeoPlot", my1st, my2nd, my3rd, my4th, "", my6th, my5th)

        ElseIf myAction = "LoadJobs" Then
        ...
        ElseIf myAction = "LoadMarkerData" Then

            myReturn = f_LoadMarkerData(my1st)

        ElseIf myAction = "LoadJobData" Then

            Dim myPartyID As String = CustGenClass.f_Encrypt(my1st)
            Dim myAdID As String = CustGenClass.f_Encrypt(my2nd)
            myReturn = "../Carrier/CarrierSnapshot.aspx?Close=Yes&RowOwnerCIX=" & myPartyID & "&Party=" & myPartyID & "&Ad=" & myAdID & "&Lat=" & my3rd & "&Long=" & my4th
    ...
    Protected Sub RadAjaxManager1_AjaxRequest(ByVal sender As Object, ByVal e As Telerik.Web.UI.AjaxRequestEventArgs) Handles RadAjaxManager1.AjaxRequest
        ...
        If myAction = "rebindReviewsRG" Then
            ReviewsRG.DataBind()
    ...
    Protected Sub SearchForTruckerRB_Click() Handles SearchForTruckerRB.Click

        PickTruckerRG.Rebind()

    End Sub

    Public Sub PostingCitiesRAC_DataSourceSelect(ByVal sender As Object, ByVal e As AutoCompleteBoxDataSourceSelectEventArgs)
    ...
    Public Sub s_SaveRACs() Handles SaveRB.Click

        Dim myPartyID As String = Me.HiddenTB_PartyID.Text
        Dim myTruckerID As String = Me.HiddenTB_TruckerID.Text
        Dim myReviewedAdID As String = Me.HiddenTB_PK.Text
        Dim myCityID As String = ""
        Dim myPosLat As String = Me.HiddenTB_CenterLat.Text
        Dim myPosLong As String = Me.HiddenTB_CenterLong.Text
        Dim myPostingSources As String = ""

        For Each entry As AutoCompleteBoxEntry In JobFoundRAC.Entries
            Dim myEntry As String = entry.Value
            myPostingSources = myPostingSources & myEntry & ","
        Next

        For Each entry As AutoCompleteBoxEntry In PostingCitiesRAC.Entries
            myCityID = entry.Value
        Next

        Dim myReturn As String = CustGenClass.f_Sproc("usp_CheckIn_PostingSource", myPartyID, myTruckerID, myReviewedAdID, myCityID, myPosLat, myPosLong, myPostingSources)
        RadAjaxManager1.ResponseScripts.Add("onSaveRB_Clicked();")

    End Sub

    Protected Sub DashboardRG_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles DashboardRG.ItemDataBound

        If TypeOf e.Item Is GridDataItem Then

            Dim dataItem As GridDataItem = CType(e.Item, GridDataItem)
            Dim myPOICat As String = dataItem.GetDataKeyValue("POICat").ToString
            Dim myThumbnailRBI As RadBinaryImage = CType(dataItem("ResultsColumn").Controls(5), RadBinaryImage)
            Dim myViewSourceRB As RadButton = CType(dataItem("ResultsColumn").Controls(3), RadButton)
            Dim myHaveRole As String = Me.HiddenTB_HaveRole.Text

            If myPOICat <> "Job" Then
                myThumbnailRBI.Visible = False
                myViewSourceRB.Visible = False
            Else
                myThumbnailRBI.Visible = True
                If myHaveRole <> "Trucker" Then
                    myViewSourceRB.Visible = True
                Else
                    myViewSourceRB.Visible = False
                End If
            End If
    ...
    Public Sub FastportP2RB_Click() Handles FastportP2RB.Click

        Dim myTruckerID As String = CustGenClass.f_Encrypt(Me.HiddenTB_TruckerID.Text)
        Response.Redirect("../Driver/FastportConfig.aspx?Party=" & myTruckerID)
    ...
    Public Sub s_P2_Instructions(ByVal myRole As String, ByVal myStatus As String)

        If myRole = "Trucker" Then
            s_TruckerPanel_Instructions(PrefsInstructionsLbl, "934")
        Else
            s_TruckerPanel_Instructions(PrefsInstructionsLbl, "936")
            If myStatus = "Search" Then
                s_TruckerPanel_Instructions(GeneralInstructionsLbl, "935")
            ElseIf myStatus = "Adding" Then
                s_TruckerPanel_Instructions(GeneralInstructionsLbl, "941")
            ElseIf myStatus = "Locked" Then
                s_TruckerPanel_Instructions(GeneralInstructionsLbl, "942")
            ElseIf myStatus = "Editing" Then
                s_TruckerPanel_Instructions(GeneralInstructionsLbl, "943")
            ElseIf myStatus = "Managed" Then
                s_TruckerPanel_Instructions(GeneralInstructionsLbl, "944")
            End If
        End If

    End Sub

    Public Sub s_TruckerPanel_Instructions(ByVal myLbl As Label, ByVal myConfigID As String)

        Dim myW As String = FlowConfigTable.ConfigID.UniqueName & " = " & myConfigID
        Dim myR As FlowConfigRecord = FlowConfigTable.GetRecord(myW)
        myLbl.Text = myR.Instructions

    End Sub

    'On load, if trucker, load fields on page load.
    'On load, If trucker, disable fields on page load.
    'On load, set instructions for all.

    'After search, populate data,
    'After search, disable fields.
    'After search, set buttons based on roles.
    'After search, enable panels.

    'On new click, clear fields.
    'On new click, enable all fields.
    'On new click, set buttons.
    'On new click, disable panels.

    'On new save, create records.
    'On new save, 



    Public Function f_LoadMarkerData(ByVal PK As String) As String

        Dim myWhere As String = V_PartyOrgView.AddrID.UniqueName & " = " & PK
        Dim myRec As V_PartyOrgRecord = V_PartyOrgView.GetRecord(myWhere)

        Dim POI As String = myRec.PointOfInterest
        Dim ContactInfo As String = myRec.ContactInfo
        Dim Pins As String = myRec.PinsSmall
        Dim PointStats As String = myRec.PointStats

        Dim myReturn As String = POI & "<|>" & ContactInfo & "<|>" & Pins & "<|>" & PointStats
        ReviewsRG.DataBind()
    ...
    Protected Overridable Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim myMeID As String = Me.HiddenTB_PartyID.Text
        Dim myReturn As String = CustGenClass.f_Sproc("usp_LastCheckIn", myMeID)
        Dim myOriginLat As String
        Dim myOriginLong As String
        If myReturn <> "0" Then
            myOriginLat = CustGenClass.f_Split_ByComma(myReturn, 1)
            myOriginLong = CustGenClass.f_Split_ByComma(myReturn, 2)
        Else
            myOriginLat = "0"
            myOriginLong = "0"
        End If
        Me.HiddenTB_OriginLat.Text = myOriginLat
        Me.HiddenTB_OriginLong.Text = myOriginLong
