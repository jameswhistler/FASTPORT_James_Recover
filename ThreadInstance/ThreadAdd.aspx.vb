...
' This file implements the code-behind class for ThreadAdd.aspx.
' ThreadAdd.Controls.vb contains the Table, Row and Record control classes
...
Partial Public Class ThreadAdd
        Inherits BaseApplicationPage
' Code-behind class for the ThreadAdd page.
' Place your customizations in Section 1. Do not modify Section 2.
        
#Region "Section 1: Place your customizations here."


        Protected WithEvents RadUpload1 As RadUpload
        Protected WithEvents MessageSubject As RadTextBox
        Protected WithEvents ThreadRE As RadEditor
        Protected WithEvents ToRAC As RadAutoCompleteBox


        Public Sub SendButton_Click(ByVal sender As Object, ByVal args As EventArgs)

            ' Order of committing work
            ' ------------------------

            ' 1) Update the ThreadInstanceMessage row
            ' 2) Pass email list (creates ThreadInstanceParties)
            ' 3) Create ThreadInstanceMessageLog entries
            ' 4) Upload attachments (last because this is arguably the least critical)

            Dim myRadUploadedFilesCount As Integer = RadUpload1.UploadedFiles.Count

            Dim myUserID As String = CType(Me.Page, BaseApplicationPage).SystemUtils.GetUserID()
            Dim myInstanceID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Instance"))
            Dim myMessageID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Message"))
            Dim myNewMessageID As Integer
            ...
            Dim myComment As String = ThreadRE.Content
            Dim mySubject As String = MessageSubject.Text

            If String.IsNullOrEmpty(myMessageID) Then

                If Not String.IsNullOrEmpty(FlowStep) Then
                    myMessageID = FlowBaseClass.f_URL_Email_ByButton(FlowStep, , , Execution, AdActivity, RowOwnerCIX, RowOIX, Party, FleetCircle, myLeftPart)
                    CustGenClass.s_UpdateContent(myMessageID, myComment, mySubject)
                Else
                    ...
                    myMessageID = CustGenClass.f_Sproc("usp_ThreadMessageAdd", myInstanceID, myComment, myUserID, "Yes", mySubject)
                    CustGenClass.s_URL_Update(myMessageID, , myLeftPart, , , , , RowOwnerCIX, RowOIX, FlowStep, Party, myUserID, Message, Instance, FleetCircle, Execution, , AdActivity, CheckIn, DocFiled, , , , , , , , , , , , , , , Verification, Doc)
            ...
            myNewMessageID = CInt(myMessageID)

            ' 2) Pass email list (creates ThreadInstanceParties)

            Dim myWarning As String = ""

            For Each entry As AutoCompleteBoxEntry In ToRAC.Entries

                Dim myToValue As String = entry.Value

                If String.IsNullOrEmpty(myToValue) Then
                    myToValue = "New," & entry.Text
                End If

                Dim mySproc As String = CustGenClass.f_Sproc("usp_ThreadRecipient_AddOne", myInstanceID, myUserID, myToValue)

                If mySproc <> "Success" Then

                    myWarning = myWarning & mySproc & ", "

                End If

            Next

            If Not String.IsNullOrEmpty(myWarning) Then

                myWarning = "WARNING: The following emails are not valid: " & Left(myWarning, Len(myWarning) - 2) & "."
            ...
            Try
                ...
                Dim mySprocResult As String = CustGenClass.f_Sproc("usp_ThreadMessageLogAdd", CType(myNewMessageID, String))
            ...
            Try
                ...
                If RadUpload1.UploadedFiles.Count > 0 Then
                    For Each file As UploadedFile In RadUpload1.UploadedFiles
                        Dim myThreadInstanceMessageAttachmentsRec As ThreadInstanceMessageAttachmentsRecord = New ThreadInstanceMessageAttachmentsRecord
                        myThreadInstanceMessageAttachmentsRec.MessageID = myNewMessageID
                        myThreadInstanceMessageAttachmentsRec.FileName = file.GetName()
                        'FIX
                        'Dim bytes(file.ContentLength - 1) As Byte
                        'file.InputStream.Read(bytes, 0, file.ContentLength)
                        'myThreadInstanceMessageAttachmentsRec.Attachment = bytes
                        myThreadInstanceMessageAttachmentsRec.Save()
                    Next
                End If
                DbUtils.CommitTransaction()
            Catch ex As Exception
            ...
            'JAR -- Updated related tables with related ids

            Dim myExecutionID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Execution"))
            Dim myAdActivityID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("AdActivity"))
            Dim myFiledID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Filed"))
            Dim myVerificationID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Verification"))
            Dim myCheckInID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("CheckIn"))
            Dim myDocID As String = CustGenClass.f_Decrypt(Me.Page.Request.QueryString("Doc"))

            CustGenClass.s_InstanceRelatedIDsUpdate(myInstanceID, myExecutionID, myAdActivityID, myFiledID, myVerificationID, myCheckInID, myDocID)

            'JAR -- Send "Edited" items to send.
        ...
        Public Sub s_Vis()
            ...
            ' Load the data from the database into the DataSource ThreadInstanceMessage record.
            ...
            If Not String.IsNullOrEmpty(myInstanceID) Then
                MiscUtils.FindControlRecursively(Me, "ExistingAddressesRow").Visible = True
                Dim mySprocResult As String = CustGenClass.f_Sproc("usp_ThreadRecipientString", myInstanceID)
                If mySprocResult <> "0" Then
                    Me.ExistingAddresses.Text = mySprocResult
                Else
                    Me.ExistingAddresses.Text = ""
                End If
            Else
                MiscUtils.FindControlRecursively(Me, "ExistingAddressesRow").Visible = False
            End If

            ' Show the RadComboBox1 for editting
            Dim mySubject As String = Me.Page.Request.QueryString("Subject")
            If Not String.IsNullOrEmpty(mySubject) Then
                mySubject = HttpUtility.UrlDecode(mySubject)
                MessageSubject.Text = mySubject
            End If

            Dim myBody As String = Me.Page.Request.QueryString("Body")
            If Not String.IsNullOrEmpty(myBody) Then
                myBody = HttpUtility.UrlDecode(myBody)
                ThreadRE.Content = myBody
            End If
            ...
            If Not String.IsNullOrEmpty(myMessageID) Then
                ...
                If Not IsNothing(myR) Then
                    If Not String.IsNullOrEmpty(myR.MessageSubject) Then
                        MessageSubject.Text = myR.MessageSubject
                    End If
                    If Not String.IsNullOrEmpty(myR.MessageBody) Then
                        ThreadRE.Content = myR.MessageBody
                    End If
                End If

                myW = ThreadInstanceMessageAttachmentsTable.MessageID.UniqueName & " = " & myMessageID
                Dim myAttachR As ThreadInstanceMessageAttachmentsRecord = ThreadInstanceMessageAttachmentsTable.GetRecord(myW)
                If Not IsNothing(myAttachR) Then
                    MiscUtils.FindControlRecursively(Me, "CurrentAttachmentsRow").Visible = True
                End If
...
#Region "Section 2: Do not modify this section."
        ...
        Public WithEvents AddAddressesLiteral As System.Web.UI.WebControls.Literal
        Public WithEvents AttachmentsLiteral As System.Web.UI.WebControls.Literal
        Public WithEvents CancelButton As ThemeButton
        Public WithEvents CurrentAttachmentsLiteral As System.Web.UI.WebControls.Literal
        Public WithEvents ExistingAddresses As System.Web.UI.WebControls.Literal
        Public WithEvents ExistingAddressesLiteral As System.Web.UI.WebControls.Literal
        Public WithEvents PageTitle As System.Web.UI.WebControls.Literal
        Public WithEvents SendButton As ThemeButton
        Public WithEvents SubjectLiteral As System.Web.UI.WebControls.Literal
        Public WithEvents ThreadInstanceMessageAttachmentsTableControl As FASTPORT.UI.Controls.ThreadAdd.ThreadInstanceMessageAttachmentsTableControl
        Public WithEvents ToDS As System.Web.UI.WebControls.SqlDataSource
    ...
    End Function
      ...
      Public Sub SaveData_Base()
        ...
        Me.ThreadInstanceMessageAttachmentsTableControl.SaveData()
      ...
      End Sub  
        ...
        Me.ThreadInstanceMessageAttachmentsTableControl.LoadData()
        Me.ThreadInstanceMessageAttachmentsTableControl.DataBind()
